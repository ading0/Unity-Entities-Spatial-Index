...
namespace SpatialHashing
{
    ...
    public struct VoxelRay<T> where T : IRay
    {
        ...
        public bool RayCast(ref T asker, float3 start, float3 direction, float length)
        {
            ...
            int3 currentVoxel = asker.GetIndexVoxel(start);

            int voxelDistance = ComputeVoxelDistance(ref asker, start + direction * length, currentVoxel);

            // In which direction the voxel ids are incremented.
            int3 directionStep = GetSignZeroPositive(direction);

            // Distance along the ray to the next voxel border from the current position (max.x, max.y, max.z).
            float3 nextVoxelBoundary = asker.GetPositionVoxel(currentVoxel + GetNegativeSign(directionStep) + 1, false);
         ...
         */
        private readonly int ComputeVoxelDistance(ref T asker, float3 end, int3 startIndex)
        {
